...
export async function simulatePaymentFailure(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    logger.info(`[Demo Controller] Simulating payment failure for ${name} (${sample.category})`);

    const result = await webhookService.processWebhook(payload.event_id, payload.event, payload.payload);
...
export async function executePendingRetries(req: Request, res: Response, next: NextFunction) {
  try {
    const { workflowId } = req.body;

    const whereClause = workflowId ? { id: workflowId } : { status: 'ACTIVE' };

    const activeWorkflows = await prisma.recoveryWorkflow.findMany({
      where: whereClause,
      include: {
    ...
    for (const workflow of activeWorkflows) {
      ...
      const recovery = await prisma.revenueRecovery.create({
        data: {
          transactionId: workflow.transactionId,
...
export async function resetDemoData(req: Request, res: Response, next: NextFunction) {
  try {
    logger.info('[Demo Controller] Resetting database demo data...');
