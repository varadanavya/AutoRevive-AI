...
class NotificationService {
    ...
    async sendNotification(payload) {
        ...
        const recoveryLink = `http://localhost:${config_1.config.port}/#recover-${payload.paymentId}`;
        let messageBody = '';
        switch (payload.type) {
        ...
        logger_1.logger.info(`[Notification Service] Dispatching ${payload.channel} (${payload.type}) to ${payload.recipient}: "${messageBody}"`);
        // Log dispatch event to Prisma database
        const notification = await db_1.prisma.notificationLog.create({
            data: {
     ...
     */
    async triggerCustomerActionCampaign(params) {
        logger_1.logger.info(`[Notification Service] Launching Multi-Channel Customer Recovery Campaign for ${params.customerName}`);
